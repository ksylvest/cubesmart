...
RSpec.describe CubeSmart::CLI do
  ...
  describe '#parse' do
    ...
    context 'with a crawl command' do
      it 'proxies CubeSmart::Facility.crawl' do
        allow(CubeSmart::Facility).to receive(:crawl)
        expect { cli.parse(%w[crawl]) }.to raise_error(SystemExit)
        expect(CubeSmart::Facility).to have_received(:crawl)
      end
    end
    ...
    context 'with a version flag' do
      %w[-v --version].each do |option|
        it "prints version with '#{option}'" do
          ...
          expect(cli).to have_received(:puts).with(CubeSmart::VERSION)
        end
      end
    end
